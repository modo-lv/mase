...
fun ByteArray.findChecksumSegments(sections: Sections): ChecksumSegments {
    ...
    val playerLoc = this.readPlayerLocation()
    var levelMapIndex = 0
    ...
    for (location in 1 ..< 51) { // Last checksum is added manually
        for (level in 0 ..< 100) {
            val visited =
                this.readLeInt(Location.Addresses.LOCATION_VISITED_FLAGS_BASE + (400 * location) + (level * 4)) != 0
            val type = this.readLeInt(levelInfoAddresses[location * 100 + level] + Section.HEADER_SIZE)
            if (visited || (
                        // Type meaning unknown, values copied from savadomer (https://gitlab.com/mikesc/savadomer)
