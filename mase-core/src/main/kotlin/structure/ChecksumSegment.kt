...
typealias ChecksumSegments = List<ChecksumSegment>
...
class ChecksumSegment(val range: IntRange, val xorValue: UInt) {
     ...
     */
    var computedChecksum: UInt by notNull()
     ...
     * Trying to read this before running [compute] at least once will throw an exception (see [notNull]).
     */
    var storedChecksum: UInt by notNull()

    /**
     * Indicates whether this segment's [computedChecksum] is different from the [storedChecksum] in the save data.
     */
    fun isMismatched(): Boolean = computedChecksum != storedChecksum

    /**
     * Computes the CRC hash for the data in this segment, and stored the result in [computedChecksum].
     *
     * @param lastChecksum Checksum from the previous segment computation. 0xFFFFFFFF for the first segment.
    ...
    fun compute(lastChecksum: UInt, saveData: ByteArray, isLast: Boolean = false): UInt {
        ...
        readStored(saveData)
        logger.debug { "Computed hash for ${this.range.toHex()} is ${hash.toHex()}, file: ${storedChecksum.toHex()}" }
    ...
    companion object {
        const val CHECKSUM_SIZE = 20 // Checksum + its 4 derivatives
